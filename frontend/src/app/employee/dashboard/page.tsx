...
export default function EmployeeDashboardPage() {
  ...
  const { data: profile } = useSWR("/employees/me", fetchEmployeeProfile, { revalidateOnFocus: false });
  const { data: summary, mutate: mutateSummary } = useAttendanceSummary({ type: "employee", month });
  const { data: records, mutate: mutateAttendance } = useAttendance({ type: "employee", month });
  const { data: salary, mutate: mutateSalary } = useSalary({ type: "employee", month });
  const { data: advances } = useAdvancePayments({ type: "employee", month });
  ...
  async function handleCheckIn() {
    ...
    try {
      await checkInAttendance({ type: "employee", notes: "Checked in from employee dashboard" });
  ...
  async function handleCheckOut() {
    ...
    setAttendanceError(null);
    try {
      await checkOutAttendance({ type: "employee", notes: "Checked out from employee dashboard" });
      await Promise.all([mutateAttendance(), mutateSummary(), mutateSalary()]);
  ...
  async function markAbsent() {
    ...
    try {
      await createAttendance({
        ...
        notes: "Marked absent from employee dashboard",
      });
      await Promise.all([mutateAttendance(), mutateSummary(), mutateSalary()]);
  ...
  return (
    <DashboardShell title="Employee Dashboard" subtitle="Attendance, salary, and payment status in one place" items={employeeNavItems}>
      <div className="glass col-span-full p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="panel-title">{profile?.name ?? "Employee Workspace"}</h2>
            <p className="panel-subtitle">{profile?.email ?? "Loading your profile..."}</p>
          </div>
      ...
      <div className="glass p-6">
        ...
        <p className="panel-subtitle">Current day status and timings.</p>
      ...
      <div className="glass p-6">
        ...
        <p className="panel-subtitle">Attendance is tracked separately for {month}.</p>
      ...
      <div className="glass p-6">
        ...
        <p className="panel-subtitle">Your advance payments for the selected month.</p>
      ...
      <div className="glass p-6">
        ...
        <p className="panel-subtitle">Latest salary payment record.</p>
        <div className="mt-5 space-y-3 text-sm text-indigo-100/75">
