...
export default function RiderAttendancePage() {
  ...
  const { data: attendance, error, mutate } = useAttendance({ type: "rider", month });
  ...
  async function handleCheckIn() {
    ...
    setSubmitError(null);
    try {
      await checkInAttendance({ type: "rider", notes: "Checked in from rider attendance page" });
      await mutate();
  ...
  async function handleCheckOut() {
    ...
    setSubmitError(null);
    try {
      await checkOutAttendance({ type: "rider", notes: "Checked out from rider attendance page" });
      await mutate();
  ...
  async function markAbsent() {
    ...
    try {
      await createAttendance({
        ...
        notes: "Marked absent from rider attendance page",
      });
      await mutate();
  ...
  return (
    <DashboardShell title="Attendance" subtitle="Check in, check out, and review your rider attendance" items={riderNavItems}>
      <div className="glass p-6">
        <h2 className="panel-title">Today's Status</h2>
        <p className="panel-subtitle">Aaj ki rider attendance yahin se manage hogi.</p>
      ...
      <div className="glass col-span-full p-6">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="panel-subtitle">Present, absent, and check-out records with timings.</p>
          </div>
        ...
        <div className="mt-6 overflow-hidden rounded-xl border border-white/10">
          <table className="w-full text-left text-sm text-indigo-100/80">
            ...
            <tbody>
              ...
              {!attendance?.length && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-sm text-indigo-100/70">No attendance records yet.</td>
                </tr>
              )}
            </tbody>
          </table>
